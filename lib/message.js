...
async function MessagesUpsert(clutch, message, store) {
	try {
		...
		if (global.db.settings.autoread && global.db.settings.autoread == true) clutch.readMessages([msg.key])
		if (global.db.settings.autotyping && global.db.settings.autotyping == true && !msg.key.fromMe) clutch.sendPresenceUpdate('composing', msg.key.remoteJid)
		if(global.db.settings.autorecord && global.db.settings.autorecord == true && !msg.key.fromMe) clutch.sendPresenceUpdate('recording', msg.key.remoteJid)
...
async function Solving(clutch, store) {
	...
	clutch.getName = (jid, withoutContact  = false) => {
		const id = clutch.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || clutch.groupMetadata(id) || {};
